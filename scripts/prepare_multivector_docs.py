...
PROCESS_PDF = True      # Tạm thời False để debug HTML. Khi ổn thì đổi True.
PROCESS_HTML = True

PDF_DIR = Path("data/raw/pdf")
HTML_DIR = Path("data/raw/html")

OUT_DIR = Path("data/processed/multivector_preview")
IMAGE_DOWNLOAD_DIR = Path("data/processed/downloaded_images")
...
def download_remote_image(src: str, file_name_prefix: str) -> Path | None:
    try:
        ...
        out_path = IMAGE_DOWNLOAD_DIR / f"{file_name_prefix}_{uuid.uuid4().hex}{suffix}"
